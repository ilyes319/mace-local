...
def get_edge_triplets(
    edge_index: torch.Tensor,
    num_nodes: int,
) -> torch.Tensor:
    row, col = edge_index[0, :], edge_index[1, :]  # j->i

    value = torch.arange(1, row.size(0) + 1, device=row.device)
    adj_t = torch.sparse_coo_tensor(
        indices=edge_index, values=value, size=(num_nodes, num_nodes)
    )
    adj_t_dense = adj_t.to_dense().T
    adj_t_row = adj_t_dense[row, :].to_sparse()
    adj_t_dense_ones = adj_t_row.to_dense()
